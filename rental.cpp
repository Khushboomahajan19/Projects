...
class Cars
{
    ...
    void entry()
    {
        cout << "Enter company name              : "; fflush(stdin); cin.getline(company_name, 18);
        cout << "Enter model name                : "; fflush(stdin); cin.getline(model_name, 18);
        cout << "Enter plate number              : "; fflush(stdin); cin.getline(plate_no, 12);
        cout << "Capacity (5 or 7 seater)        : "; fflush(stdin); cin >> seater;
        cout << "Fuel(P petrol, D diesel, C CNG) : "; fflush(stdin); cin >> fuel_type;
        cout << "Color                           : "; fflush(stdin); cin.getline(color, 14);

    }
    friend bool unique_check(char plate_no[]);
    friend int showData();
};
template <class T>
class customer
{
    ...
    void getData()
    {
        ...
        cout << "Customer Name       : "; fflush(stdin); cin.getline(name, 20);
        cout << "Contact Number      : "; fflush(stdin); cin.getline(phone_no, 12);
    }
    friend void requestcar();

};
int showData(char opt)
{
    Cars show; int counting = 0;
    try
    {
        ifstream input;
        if(opt == '*')
        ...
        input.seekg(ios :: beg);
        system("CLS");
        cout << "\t Showing all available Cars." << endl << endl;
        cout << "   --------------------------------------------------------------- " << endl;
        while(input.read((char *)&show, sizeof(show)))
        {
            ...
            cout << "  |Enter company name              : "<< setw(30) << left << show.company_name << "|"  << endl;
            cout << "  |Enter model name                : "<< setw(30) << left << show.model_name << "|"  << endl;
            cout << "  |Enter plate number              : "<< setw(30) << left << show.plate_no << "|"  << endl;
...
bool unique_check(char plate_no[], char mode)
{
    Cars chek;
    try
    {
...
void updatecarsavailable(char plate_no[])
{
    Cars chek;
    try
    {
        ifstream input; ofstream temp;
        input.open("carsAvailable.dat", ios :: binary | ios :: in);
        ...
        //newrec();
        ofstream out; ifstream in; Cars copyy;
...
void startTrip(customer<float> confirm)
{
    ofstream out;
    cout << endl;
    ...
    try
    {
        ...
        updatecarsavailable(confirm.plate_no);
...
void requestcar()
{
    ...
    Cars req; int empty = 0; customer<float> cust; char choice = 'y';
    starting:
    fflush(stdout);
    //cout << "Customer is requesting for car." << endl;

    empty = showData('0');
    cout << "Customer is requestion for a car." << endl;
    if(empty != 0)
    {
        cust.getData();
        cout << "Choose plate number : "; fflush(stdin); cin.getline(cust.plate_no, 12);
        if(!unique_check(cust.plate_no, 'a'))
        {
            cout << "Car of plate number " << cust.plate_no << " is available." << endl;
            cout << "Enter Time period(in hours or in days) : "; fflush(stdin); cin >> cust.time_per;
            cout << "Enter charges(per hours or per day)    : "; fflush(stdin); cin >> cust.charges;
        ...
        cout << "Have a new customer (y or n) : "; fflush(stdin);cin >> choice;
        if(choice == 'y')
        {
            goto starting;
        }
...
int showtrips()
{
    customer<float> show; int counting = 0;
    try
    {
        ifstream input;
        ...
        input.seekg(ios :: beg);
        system("CLS");
        cout << "\t Showing all ongoing trips." << endl << endl;
        cout << "  -----------------------------------------------------------------" << endl;
        while(input.read((char *)&show, sizeof(show)))
        {
            counting++;
...
void returncar()
{
    ...
    Cars back;
    starting:
    empty = showtrips(); exist = 0; choice = 'y';
    if(empty != 0)
    {

        cout << endl << "Enter plate number of returned car : ";fflush(stdin); cin.getline(plate_no, 12);
        ifstream input;
        input.open("ongoingTrips.dat", ios :: binary | ios :: in);
        ofstream temp;
        temp.open("temporary.dat", ios :: binary | ios :: out | ios :: trunc);
        while(input.read((char *)&rtrn, sizeof(rtrn)))
        {
            ...
            {
                exist = 1;
                cout << "\tShowing details of trips." << endl << endl;
                cout << "  ----------------------------------------------------------------- " << endl;
...
void deletecar()
{
    char plate_no[15];
    showData('0');
    cout << endl << "Enter plate number : "; fflush(stdin); cin.getline(plate_no, 12);
    if(!unique_check(plate_no, '0'))
    {
        updatecarsavailable(plate_no);
        Cars chek;
        try
        {
            ifstream input; ofstream temp;
...
void addCar()
{
    Cars regis; char choice = 'y'; char ans;
    ofstream out; ofstream extra; ofstream extra2;
    system("cls");
    cout << endl;
    ...
    cout << endl;
    cout << endl;
    cout << "Would you like to delete all previous records(y or n) ?"; fflush(stdin); cin >> ans;
    if(ans == 'n')
    {
    ...
    try
    {
        ...
        do
        {
            ...
            extra.close();
            out.close();
            if(unique_check(regis.plate_no, '*'))
            {
            ...
            stop2:
            cout << "Would you like to enter new car to database(y or n): "; fflush(stdin);
            cin >> choice;
            if(choice != 'y' && choice != 'n')
            {
                cout << "\t Invalid input. Try again" << endl;
...
void adminPage()
{
    ...
    cout << "\t 7. LogOut." << endl;
    cout << endl << endl;
    cout << "\t Enter your choice :"; fflush(stdin); cin >> choice;
    switch(choice)
    {
        case '1': addCar(); break;
        case '2': showData('*'); break;
        case '3': requestcar(); break;
        case '4': returncar(); break;
        case '5': showtrips(); break;
        case '6': deletecar(); break;
        case '7': adminLogin(); break;
        default: cout << "Invalid input" << endl;cout << "Try again." << endl;system("PAUSE");
    }
